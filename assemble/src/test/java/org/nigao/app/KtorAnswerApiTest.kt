...
class KtorAnswerApiTest {
    ...
    /**
     * This endpoint sends `allow_segment_interaction` as the number 1, while `/api/v3/feed/...`
     * sends the boolean `true`. Modelled as a plain `Boolean?`, that number aborts the whole decode
     * and takes the answer down with it — the "该回答可能已删除，无法置顶显示" failure again, from one
     * flag nothing even reads. `segment_infos` rides along on the same response, so pin both.
     */
    @Test
    fun `the numeric allow_segment_interaction of this endpoint still decodes`() {
        assertTrue(
            "the fixture must carry the numeric form",
            ANSWER_BODY.contains("\"allow_segment_interaction\":1"),
        )

        val target = requireNotNull(requireNotNull(api.parseAnswer(ANSWER_BODY)).target)

        assertEquals(true, target.allowSegmentInteraction)
        assertEquals("fhaXmjAX", target.segmentInfos?.first()?.pid)
        assertEquals(2, target.segmentInfos?.first()?.marks?.first()?.startIndex)
        assertEquals(66, target.segmentInfos?.first()?.marks?.first()?.endIndex)
        assertEquals(
            listOf("2078150856671810573"),
            target.segmentInfos?.first()?.marks?.first()?.segInfo?.segIds,
        )
    }
