package org.nigao.zhihuLite.business_logic.zhihu.sign
...
object Zse96 {
     ...
     *
     * Iterates by Unicode code point, NOT by `Char`. The original per-`Char` loop encoded each
     * UTF-16 unit on its own, so a non-BMP character (any emoji, for example) had its surrogate
     * pair encoded as two lone surrogates, which `String.toByteArray(UTF_8)` turns into `??`
     * (`%3F%3F`) instead of the correct 4-byte sequence. A differential test against the original
     * JavaScript found 110/110 non-BMP inputs diverging for exactly this reason.
     *
     * Reachability note: in production this only ever encodes an ASCII MD5 hex digest
     * (see [generate]), so the defect was latent. It is fixed anyway because it silently produced
     * a wrong signature for any non-ASCII input rather than failing loudly.
     */
    private fun encodeUriComponent(s: String): String = buildString {
        var i = 0
        while (i < s.length) {
            val cp = s.codePointAt(i)
            // Character.charCount returns 2 for a surrogate pair, 1 otherwise.
            i += Character.charCount(cp)
            if (cp < 0x80 && isUriComponentSafe(cp.toChar())) {
                append(cp.toChar())
            } else {
    ...
    private fun isUriComponentSafe(ch: Char): Boolean =
...
/**
 * JavaScript `>>>`. Unsigned right shift, so the result must NOT be sign-extended.
 *
 * `Int.toLong()` sign-extends: `0xFFFFFFFF.toInt() ushr 0` is `-1` as an `Int`, and `.toLong()`
 * makes it `-1L`, whereas JavaScript yields the unsigned value `4294967295`. Masking with
 * `0xFFFFFFFFL` reproduces the JS semantics; the sibling `&`/`|`/`^`/`<<`/`>>` helpers above are
 * correct without a mask because JS also yields signed int32 for those.
 *
 * The upstream VM currently only reaches this with values whose upper bit is clear, so no shipped
 * vector exercises the difference — it is fixed to remove a silent wrong-signature failure mode.
 */
private fun jsUshr(a: Any?, b: Any?): Any? = ((toInt32(a) ushr toInt32(b)).toLong() and 0xFFFFFFFFL)
