...
interface FeedDao {
    ...
    /** Drops the feed's cursor row; the foreign key cascades to its items. */
    @Query("DELETE FROM feed_query WHERE id = :queryId")
    suspend fun deleteQueryRow(queryId: String)

    /**
     * Forget one feed completely: its rows **and** its cursor.
     *
     * Used when a screen that only cached its feed for the visit goes away. The cursor row has to go
     * with the rows: while it survives, `hasLoadedOnce` keeps answering "already loaded" and the
     * feed comes back empty instead of being fetched again.
     *
     * One transaction, so a flow observer sees the feed vanish in a single emission.
     */
    @Transaction
    suspend fun clearFeed(queryId: String) {
        deleteByQuery(queryId)
        deleteQueryRow(queryId)
    }
