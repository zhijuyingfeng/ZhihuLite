...
interface FeedStorage {
    ...
    /**
     * Delete every row of every feed, cursors included, and reclaim the file space.
     *
    ...
    /**
     * Forget one feed entirely: its rows *and* its cursor.
     *
     * Used when a screen that only cached its feed for the visit goes away. The cursor has to go
     * with the rows — leaving it behind would keep answering "already loaded" for an empty feed.
     */
    suspend fun clearQuery(query: FeedQuery)
}
...
class RoomFeedStorage(
    private val database: ZhihuDatabase,
    private val mapper: FeedMapper = FeedMapper(),
) : FeedStorage {

    private val dao: FeedDao = database.feedDao()
    ...
    override suspend fun clearAll() {
        dao.clearAllFeedData()
        compact()
    }

    override suspend fun clearQuery(query: FeedQuery) = dao.clearFeed(query.id)

    /**
     * Reclaims the space a whole-table wipe left behind.
     *
     * Measured on the platform rather than assumed: Android's SQLite runs with `auto_vacuum = FULL`
     * (verified with `PRAGMA auto_vacuum` on a Robolectric database), so **the database file is
     * already shrunk by the delete itself** — `page_count` returns to its initial value with no
     * `VACUUM` at all. What does keep its high-water mark is the **write-ahead log** (roughly 900 KB
     * after 200 rows), and only a checkpoint gives that back, so the checkpoint is the part that
     * matters here.
     *
     * `VACUUM` is kept as cheap insurance for a database where `auto_vacuum` is off (one copied in
     * from elsewhere, or a future platform change): it rewrites an empty database in one page-touch.
     * Both statements must stay **outside** a transaction — SQLite refuses `VACUUM` inside one — and
     * both are blocking, hence [Dispatchers.IO].
     *
     * A failure is logged, not thrown: the data is already gone, so failing the caller over a size
    ...
    private suspend fun compact() = withContext(Dispatchers.IO) {
        ...
        // Order matters: `VACUUM` writes its own pages through the log, so the checkpoint has to come
        // last or the log is left holding the rewrite (measured: 40 KB instead of ~0).
        try {
            connection.execSQL("VACUUM")
        } catch (e: Exception) {
            Napier.w("Could not compact the feed database: ${e.message}")
        }
        try {
            connection.query("PRAGMA wal_checkpoint(TRUNCATE)").use { it.moveToFirst() }
        } catch (e: Exception) {
            Napier.w("Could not checkpoint the feed database: ${e.message}")
        }
    }
