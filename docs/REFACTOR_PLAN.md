...
### 7.58 回答流里点正文图片也能进查看器

**先量了一个会决定设计的事实**：卡片的 `thumbnails` 与正文里的 `<img>` **不是同一批图** —— 抽 4 篇缓存回答对比，**交集 0**（host、哈希、尺寸都不同：feed 带的是另外裁剪过的封面 ✓）。所以点正文图**不能**复用封面那个列表 ✗，否则打开的是"另一张图" ✗。

**改法**

| 层 | 改动 |
|---|---|
| 路由 | `ImageViewerRoute` 增加 `imageUrl: String? = null` ✓ —— 有它=从正文点的，无它=点封面（沿用 `thumbnails` + `page` ✓） |
| 解析 | 新增纯函数 `viewerImagesFor(thumbnails, bodyUrls, imageUrl, requestedPage): Pair<List<String>, Int>` ✓：有 url → 用**正文自己的图**（按文档顺序 ✓）并以**该 url 所在下标**为起始页 ✓（即使列表里多了条目也不会开错图 ✓）；无 url → thumbnails + 请求页 ✓（越界夹紧 ✓） |
| 正文图列表 | `HtmlParser.imageUrls(html)` ✓（走已缓存的解析 ✓，文档顺序 ✓，**排除 `class="sticker"` 表情** ✓ —— 表情是文字不是可浏览的图 ✓） |
| 图片元素 | `ImageElement` 从 **`LocalImageOpenRequest`** 取回调 ✓（与视频的 `LocalVideoPlaybackRequest` 同一套机制 ✓），有回调且 `answerId` 非空时 `noRippleClickable` ✓ |
| 提供方 | `composable<QuestionDetailRoute>` 里 alongside 视频那行再 provide 一个 ✓ → `navController.navigate(ImageViewerRoute(answerId, imageUrl = url))` ✓ —— 目的地才知道导航图 ✓ |

**几条刻意的边界**：**评论面板的 `HtmlToComposeUi` 不传 `answerId`** ✓ → 评论里的图片**不可点** ✓（查看器在那儿会解析出"回答正文的图"，是错的列表 ✗）；**首页 feed 卡片只渲染摘要文本** ✓（没有正文图可点 ✓）；表情不参与 ✓。

**验证**：新增 3 + 5 条单测（正文图顺序/排除表情/无图 → 空 ✓；封面路径 ✓、正文路径按 url 定位起始页 ✓、未知 url 仍能打开 ✓、正文解析不到时退化为单张 ✓、越界夹紧 ✓）—— **总数 237 例 0 失败** ✓，debug/release/perfetto 三变体构建通过 ✓。
**未做**：真机点按本身（设备禁止注入事件 ✗）—— 风险最高的"解析出哪一批图/从哪一张开始"已由单测钉住 ✓，剩下的"点下去是否跳转"请你上手确认 ✓。

### 7.59 图片查看器改成"从缩略图放大"的转场

用户要求：点图片进入查看器时，从缩略图位置开始、到最终位置结束的缩放动画。

**做法**（轻量、可回退 ✓，不动 `ImageViewer` 内部）：目的地 `composable<ImageViewerRoute>` 的转场从 `pushEnter/popExit`（横向滑动 ✓）改成按点击图片的位置做 `scaleIn`/`scaleOut` ✓：

- 点击瞬间，图片把**自己在根坐标里的矩形**记进 `ImageOpenHint` ✓（正文图在 `ImageElement` ✓、卡片封面在 `ImageGallery` ✓，两处都记 ✓）
- 纯函数 `imageZoomFrom(rect, screenSize)` ✓ 把它换算成"缩放比 + 变换原点" ✓：`scale = rect.width / screen.width` ✓（宽度比就够 ✓ —— 两端都保持图片自身宽高比：正文图是全宽按自身比例 ✓、查看器是 fit ✓）、`origin = rect.center / screen` ✓，并做夹紧 ✓；矩形或屏幕退化时返回 null ✓ → 回落到原来的 push ✓
- 目标是**整层缩放**（黑色背景/返回键/页码一起缩放 ✓）—— 视觉上像"图片从列表里长出来" ✓，代价是 chrome 也在缩放 ✗（见下）

**真机验证**（我点不了屏幕 ✗，所以用临时 harness：写入一个"缩略图在 20% 宽、中心 (0.5,0.4)"的提示并程序化导航，同时把动画放慢到 3 秒以便逐秒截图 ✓，harness 已还原 ✓）：连续帧里可以看到浅色面板**从屏幕中部的小矩形逐帧变大**，最后铺满全屏 ✓✓ —— 缩放转场生效 ✓。
**几何本身**由 4 条单测钉住（中屏缩略图 → scale 0.3/origin (0.25,0.15) ✓；与屏幕同宽 → scale 1 ✓；越界矩形 → 夹紧 ✓；退化输入 → null（回落 push）✓），**总数 241 例 0 失败** ✓，三变体构建通过 ✓。

**已知取舍 / 未做**：① 整层缩放（chrome 一起 ✗）—— 要做到"只有图片缩放、chrome 淡入"就得在 `ImageViewer` 内部自己做 `Animatable` ✓，代码更多 ✓；② 返回时的动画同样用这个提示 ✓（近似回到原处 ✓，不是逐像素共享元素 ✓）；③ **"真实点按是否记下正确矩形"这一步我无法自动化** ✗（设备禁止注入事件 ✓）—— 请你上手点一下确认 ✓，若起点不对（比如偏上/偏下）多半是 `boundsInRoot` 与窗口坐标的偏移（状态栏高度量级 ✓），我再校准 ✓。

### 7.60 上一轮那个转场其实没提交：两个原因，一个在我

用户反馈"编译后仍然是 push 动画，Feed 和回答列表都是"。查下来**两个原因，都不是他的构建问题** ✗：

1. **我的提交漏了这次改动本身** ✗✗：`9d555d0` 的文件清单里**没有 `AssembleAppNavHost.kt`** —— 我为了撤掉临时 harness 用了 `git checkout -- <文件>` ✗，而 harness 恰好补在**同一个文件**里 ✓，于是连转场一起撤掉了 ✗，紧接着的提交"名为转场、实则只有其余 4 个文件" ✗。备份 `/tmp/NAV_ZOOM.bak` 里还留着完整改动 ✓，已恢复 ✓。
   **教训**：撤 harness 之前先确认真实改动已经**提交**（本轮就是这么做的 ✓）；提交后也用 `git show --stat` 核对"文件清单里有它" ✓ —— 这条正好也是用户上一问"上个改动改的是啥"逼出来的核对 ✓。
2. **源页面自己也还在滑动** ✗：Navigation Compose 会**同时**播放"离开页面的 exit"和"进入页面的 enter" ✓，而 `MainFeedRoute`/`QuestionDetailRoute` 的 `exitTransition` 一直是 `pushExit()` ✗ → 全屏宽度的滑动和缩放一起播 ✓，**读者看到的主要是滑动** ✓。现在这两处按目标/来源判断 ✓：目标是 `ImageViewerRoute` → `fadeOut()` ✓；从查看器返回 → `fadeIn()` ✓，其余路由保持原来的 push ✓。

**真机验证**（临时 harness 驱动提示 + 把动画放慢到 3 秒才截得到 ✓，harness 已还原 ✓）：

| 帧 | 列表最左文字 x | 说明 |
|---|---|---|
| 转场中（第 5-6 帧） | **64 → 68** | **原地变暗** ✓（滑动会趋向 0 ✗） |
| 转场后 | 无（已淡尽 ✓） | 列表淡出、查看器放大 ✓✓ |

**测试**：241 例 0 失败 ✓；debug/release/perfetto 构建通过 ✓。

### 7.61 退出改成"向右拖动、跟着手指缩小、松手落回原处"

需求：向右滑时图片跟随手指 ✓、宽高随**向右位移**减小（**最小 10%** ✓）、松手时执行缩放动画回到图片原来所在的位置 ✓。

**做法**

| 处 | 实现 |
|---|---|
| 手势位置 | 放在**页面内层**（`ZoomableImage` 的手势循环 ✓）—— 不能放外层 ✗：`HorizontalPager` 是子节点、会先消费横向拖动 ✓；内层**消费**即可压过它（与已有缩放/平移同一套 ✓） |
| 方向 | **只吃向右** ✓：`panChange.x > 0` 且横向分量大于纵向 ✓；向左不消费 ✓ → 仍是翻到下一张（分页保留 ✓） |
| 跟随 | `dismissX += panChange.x` ✓ → `translationX = offsetX + dismissX` ✓（跟随手指 ✓） |
| 缩小 | `dismissScale = dismissScaleFor(dismissX, 屏宽)` ✓ = `clamp(1 - dx/屏宽, 0.1, 1)` ✓（纯函数 ✓）→ `graphicsLayer` 里 `scale = 缩放 × dismissScale` ✓ |
| 背景 | 我把黑底 alpha 也按同一进度淡出 ✓（需求没写 ✓，但这样"缩向缩略图"才看得出来 ✓，列表会透出来 ✓） |
| 松手 | `dismissTargetFor(hint, 屏宽)` ✓ 算出"落到缩略图"的位置（`(originX-0.5)*屏宽` ✓ + 缩略图的 scale ✓）→ 220ms 动画过去 ✓ → **再** `onDismiss()` ✓ |
| 避免叠加 | 拖完再 pop 时，目的地的 `popExitTransition` 必须**不播** ✓（否则它从全屏状态开始 ✗ = 跳一下 ✗）→ `markDismissedByDrag()` / `consumeDismissedByDrag()` ✓，命中时返回 `ExitTransition.None` ✓；点返回键的路径不变 ✓（仍是缩放回去 ✓） |

**一个按字面实现的点**：**松手就一定退出** ✓（需求只说"松手时执行缩放动画回到原位" ✓，没提阈值 ✓）。若你希望"拖得少就弹回、不退出"，那是加一行阈值判断的事 ✓，说一声 ✓。

**验证**：`ImageZoomTest` 增至 **7 例 0 失败**（拖动缩放：0 → 1 ✓、半屏 → 0.5 ✓、越界 → 0.1 ✓；落点：hint 的 origin 映射为平移 ✓、无 hint 则原地缩到 10% ✓）；**总数 244 例 0 失败** ✓，debug/release/perfetto 构建通过 ✓。
**未做**：手指跟随与观感 ✗（设备禁止注入事件 ✓）—— 数学是单测保证的 ✓，请你上手试：① 右滑是否跟手 ✓ ② 最小是否停在 10% ✓ ③ 松手是否落到点进去的那张图的位置 ✓（若落点偏移，是 `boundsInRoot` 与窗口坐标的差，一行可校 ✓）。

### 7.62 回滚"进入查看器"的缩放动画

用户要求把**进入**图片查看器的动画回滚（§7.59/§7.60 那套"从缩略图放大"）。**只回滚进入** ✓：

| 处 | 现在 |
|---|---|
| `ImageViewerRoute.enterTransition` | 回到 **`pushEnter()`** ✓（原来自己写的 `scaleIn` 拆掉 ✓，随之不再使用的 `scaleIn` import 也删了 ✓） |
| `MainFeedRoute` / `QuestionDetailRoute` 的 `exitTransition` | 回到**无条件 `pushExit()`** ✓ —— 那条"目标是不是查看器"的条件判断当初只为给**进入缩放**让路而存在 ✓，进入不再缩放，它就没有理由了 ✓ |

**保留未动的部分**（并说明为什么）：

- **退出侧的缩放** ✓：`popExitTransition` 仍是 `scaleOut(到 hint)` ✓（点返回键时缩回原处 ✓）—— 因为这与用户自己设计的**拖动退出**是同一个落点 ✓（`ExitTransition.None` 的抑制也保留 ✓，避免拖动完再叠一层 ✓）
- `MainFeedRoute` / `QuestionDetailRoute` 的 **`popEnterTransition` 条件淡入** ✓：从查看器返回时列表**淡入**而不是滑入 ✓ —— 无论退出是拖动还是返回键 ✓，图片都刚"落"回缩略图 ✓，列表再横向滑进来会与它打架 ✓

**如果**你连退出侧的缩放也想去掉（返回键改成普通横向滑动 ✓），那就是删掉那三行（`popExitTransition` 的 `scaleOut` 分支 + 两处 `popEnterTransition` 条件 ✓），说一声即可 ✓。

**验证**：244 例 0 失败 ✓；debug/release/perfetto 构建通过 ✓；干净构建已装回设备 ✓。转场本身仍需你上手确认（设备禁止注入事件 ✓）。

### 7.63 图片下方那一大块空白：feed 的懒加载占位被当成图片画了

用户问"为什么当前这张卡图片下方间距这么大"。**根因是 payload 的写法** ✓，不是布局 ✗：

feed（`/api/v3/feed/topstory/recommend`）里的图片是**懒加载**的 —— 真正要显示的 `<img>` 的 `src` 是一个 **`data:image/svg+xml`** 占位（按真实图片尺寸生成：`width=1418 height=1179` ✓），**真实地址**在 `<noscript>` 里那张 `<img>` 上（另有 `data-original` / `data-actualsrc` ✓）。渲染器只看 `src` ✗ → Coil 去画那个 SVG ✗ → 得到一个 **1418×1179 的空白框** ✓（400dp 宽时高 **≈333dp** ✓）。这正是读者看到的"图片下方间距" ✓ —— 而上面那张真图，其实是 `<noscript>` 里那张被画出来了 ✓。

**实测核对**：截图量到的间距 **328dp** ✓ vs 声明尺寸换算的 **333dp** ✓（1418/1179、屏宽 400dp ✓），URL 用 curl 取回 **200** ✓（地址本身没问题 ✗，是 `src` 选错了 ✓）；这台设备缓存里 **19 条带正文的 feed 项中 5 条**含这种占位 ✓。

**改法**

| 处 | 改动 |
|---|---|
| `HtmlParser.imageSourceUrl(attributes)`（新 ✓） | 取"值得加载的地址" ✓：`src` 是 `http` → 用它 ✓；`src` 是 `data:` → **返回 null** ✓（跳过 ✓，因为旁边 `<noscript>` 那张真图会被画出来 ✓，若改用它反而会**重复出两张** ✗）；完全没有 `src` 时才回退 `data-actualsrc` / `data-original` ✓ |
| `ImageElement` | `src == null` → **不渲染** ✓（不再占位 ✓）；`src` 一律走上面的函数 ✓ |
| `imageUrls`（查看器的正文列表 ✓） | 同样过滤 ✓ → 列表里是真图 ✓，点击的 url 与之一致 ✓ |

**验证**：新增 2 条单测（占位 → null ✓ 且不进 `imageUrls` ✓；真 src 优先 ✓、缺 src 时回退属性 ✓）→ **总数 246 例 0 失败** ✓；`adb input swipe` 现在可用 ✓，真机滚过带图的卡：**照片紧接下一行文字，空白消失** ✓✓。

### 7.64 真机验证：正文图可点 + 右滑退出（以及 `adb input` 现在可用）

**环境更正** ✗：此前记录的"设备禁止注入事件 ✗"**不再成立** —— 这台设备上 `adb shell input tap/swipe` 可用 ✓✓。以后触摸类改动可以自己验证 ✓（本轮就是 ✓），"留给用户手测"只应作为补充 ✓。

**用触摸实测通过的两项**（此前都只做了单测 + 帧分析 ✓）：

| 项 | 操作 | 结果 |
|---|---|---|
| 正文图可点 + 列表正确（§7.58） | 在回答正文的照片上 `input tap` | 查看器打开 ✓，且显示的是**点的那一张** ✓；页码 **1/2** ✓ —— 说明翻的是**正文自己的两张图** ✓，不是封面 `thumbnails`（那批是另一套图 ✓，本次实测又一次印证 §7.58 的判断 ✓） |
| 右滑退出（§7.61） | `input swipe` 向右 800px / 1.6s（后台发起 ✓，中途截图 ✓） | 中途：照片**跟着手指右移**（左边缘从 0 移到约半屏 ✓）、**同时变小**（不再满宽 ✓）、黑底**淡出**透出列表 ✓；松手 2.5s 后：查看器**已关闭**，回到列表 ✓ |

**顺带**：这也复核了 §7.63 的修复 ✓ —— 用于点击的那张照片本身正是 feed 的懒加载图 ✓（`<noscript>` 里那张被画出来 ✓，占位不再画 ✓）。
