package org.nigao.zhihuLite.model.video
...
data class VideoPlayInfo(
        ...
        return videoPlay.playlist.mp4
            ...
            .sortedWith(
                compareBy<VideoItem>(
                    ...
                    { it.bitrate.takeIf { bitrate -> bitrate > 0 } ?: Double.MAX_VALUE },
                    { it.size.takeIf { size -> size > 0 } ?: Long.MAX_VALUE },
                )
            )
    ...
    /**
     * First playable source, or `null` when the response carries no usable entry.
     *
     * The previous implementation used `first { it.url.isNotEmpty() }`, which throws
     * `NoSuchElementException` on an empty/malformed playlist — a crash driven entirely by server
     * data. Callers get `null` instead and show a failed state.
     */
...
/**
 * One entry of `video_play.playlist.mp4`.
 *
 * The numeric types are the ones the endpoint actually sends, not the ones that look natural:
 * `bitrate` arrives as a **fractional** value (`301.201`), so declaring it `Int` made
 * kotlinx.serialization reject the whole payload ("Unexpected symbol '.' in numeric literal") and
 * the player showed its failed state for every video. `size` is bytes, so it is a `Long` — a long
 * enough video would overflow an `Int`. A `Double` field still accepts an integral literal, which
 * makes it the tolerant choice here.
 */
@Serializable
